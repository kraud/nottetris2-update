function gameA_load()
    ...
    world:setCallbacks(beginContactA)
    -----------
...
function gameA_draw()
    ...
    if cuttingtimer == lineclearduration then
        ...
        for i = 1, #tetricutimg do
            if pause == false then
                love.graphics.draw(tetricutimg[i], tetricutpos[i * 2 - 1] * physicsscale, tetricutpos[i * 2] *
                    physicsscale, tetricutang[i], 1, 1, piececenter[tetricutkind[i]][1] * scale,
                    piececenter[tetricutkind[i]][2] * scale)
            end
        end
    ...
    if scoreaddtimer < scoreaddtime then
        if fullscreen then
            love.graphics.setScissor(105 * scale + fullscreenoffsetX, 35 * scale + fullscreenoffsetY, 55 * scale, 9 *
                scale)
        else
        ...
        love.graphics.print("+" .. lastscoreadd, 136 * scale + offsetX, 36 * scale - scoreaddtimer / scoreaddtime * 8 *
            scale, 0, scale)
                ...
                if #tetrishapescopy == 0 then --body empty
                    ...
                    for a = 1, numberofgroups do
                        if a == 1 then --reassign the old bodyid
                            local bx, by = tetribodies[i - ioffset]:getX(), tetribodies[i - ioffset]:getY()
                            ...
                            for b, c in pairs(tetrishapescopy) do
                                if shapegroups[b] == a then
                                    ...
                                    for var = 1, #cotable, 2 do
                                        cotable[var], cotable[var + 1] = tetribodies[i - ioffset]:getLocalPoint(
                                            cotable[var], cotable[var + 1])
                                    end
                            ...
                            backupimagedata = love.image.newImageData(tetriimagedata[i - ioffset]:getWidth(),
                                tetriimagedata[i - ioffset]:getHeight())
                            backupimagedata:paste(tetriimagedata[i - ioffset], 0, 0, 0, 0,
                                tetriimagedata[i - ioffset]:getWidth(), tetriimagedata[i - ioffset]:getHeight())
                            ...
                            local newid = highestbody() + 1
                            tetribodies[newid] = love.physics.newBody(world, bx, by, "dynamic")
                            ...
                            for b, c in pairs(tetrishapescopy) do
                                if shapegroups[b] == a then
                                    cotable = getPoints2table(tetrishapescopy[b])
                                    for var = 1, #cotable, 2 do
                                        cotable[var], cotable[var + 1] = tetribodies[newid]:getLocalPoint(
                                            cotable[var], cotable[var + 1])
                                    end
                            ...
                            tetribodies[newid]:setLinearDamping(0.5)
                            tetribodies[newid]:setBullet(true)

                            tetriimagedata[newid] = love.image.newImageData(backupimagedata:getWidth(),
                                backupimagedata:getHeight())
                            tetriimagedata[newid]:paste(backupimagedata, 0, 0, 0, 0, backupimagedata:getWidth(),
                                backupimagedata:getHeight())
                            tetriimages[newid] = padImagedata(tetriimagedata[newid])
                            tetrikind[newid] = tetrikind[i - ioffset]

                            --cut the image
                            cutimage(newid, numberofgroups)

                            --mass confusion
                            tetribodies[newid]:resetMassData()

                            local mass = tetribodies[newid]:getMass()
                            if mass < minmass then
...
function cutimage(bodyid, numberofgroups) --cuts the image of a body based on its shapes (2nd argument might be obsolete)
    ...
    for y = 0, height - 1 do
        for x = 0, width - 1 do
            ...
            local dummy1, dummy2 = tetribodies[bodyid]:getWorldPoint((x - width / 2 + .5) * (4 / scale),
                (y - height / 2 + .5) * (4 / scale))
...
function beginContactA(a, b, coll)
    ...
    if dataA[1] == 1 or dataB[1] == 1 then
        if dataA[1] ~= "left" and dataA[1] ~= "right" and dataB[1] ~= "left" and dataB[1] ~= "right" then
            if gamestate == "gameA" then
