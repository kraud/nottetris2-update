function gameBmulti_load()
    ...
    if not fullscreen then
        love.window.setMode(274 * mpscale, 144 * mpscale, { vsync = vsync, msaa = 16 })
    end
    ...
    wallfixturesp1[0] = love.physics.newFixture(wallbodiesp1, wallshapesp1[0])
    wallfixturesp1[0]:setUserData("leftp1")
    wallfixturesp1[0]:setFriction(0.0001)

    wallshapesp1[1] = love.physics.newPolygonShape(516, 0, 516, 672, 548, 672, 548, 0)
    wallfixturesp1[1] = love.physics.newFixture(wallbodiesp1, wallshapesp1[1])
    wallfixturesp1[1]:setUserData("rightp1")
    ...
    wallfixturesp1[2] = love.physics.newFixture(wallbodiesp1, wallshapesp1[2])
    wallfixturesp1[2]:setUserData("groundp1")
    ...
    wallfixturesp2[0] = love.physics.newFixture(wallbodiesp2, wallshapesp2[0])
    wallfixturesp2[0]:setUserData("leftp2")
    ...
    wallfixturesp2[1] = love.physics.newFixture(wallbodiesp2, wallshapesp2[1])
    wallfixturesp2[1]:setUserData("rightp2")
    wallfixturesp2[1]:setFriction(0.0001)

    wallshapesp2[2] = love.physics.newPolygonShape(516, 640, 516, 672, 836, 672, 836, 640)
    wallfixturesp2[2] = love.physics.newFixture(wallbodiesp2, wallshapesp2[2])
    wallfixturesp2[2]:setUserData("groundp2")
    -----------
    world:setCallbacks(beginContactBmulti)
    -----------
...
function gameBmulti_draw()
    ...
    for i, v in pairs(tetribodiesp1) do
        ...
        love.graphics.draw(tetriimagesp1[i], v:getX() * physicsmpscale, v:getY() * physicsmpscale, v:getAngle(), 1, 1,
            piececenter[tetrikindp1[i]][1] * mpscale, piececenter[tetrikindp1[i]][2] * mpscale)
    end

    if p1fail == false and nextpiecep1 then
        --Next piece
        love.graphics.draw(nextpieceimgmp[nextpiecep1], 24 * mpscale, 120 * mpscale, -nextpiecerot, 1, 1,
            piececenterpreview[nextpiecep1][1] * mpscale, piececenterpreview[nextpiecep1][2] * mpscale)
    end
    ...
    for i, v in pairs(tetribodiesp2) do
        ...
        love.graphics.draw(tetriimagesp2[i], v:getX() * physicsmpscale, v:getY() * physicsmpscale, v:getAngle(), 1, 1,
    ...
    if p2fail == false and nextpiecep2 then
        --Next piece
        love.graphics.draw(nextpieceimgmp[nextpiecep2], 250 * mpscale, 120 * mpscale, nextpiecerot, 1, 1,
            piececenterpreview[nextpiecep2][1] * mpscale, piececenterpreview[nextpiecep2][2] * mpscale)
    end
    --SCORES P1---------------------------------------
    ...
    if gamestate == "gameBmulti_results" then
        ...
        if winner == 1 then
            ...
            if jumpframe == false then
                love.graphics.draw(marioidle, mariobody:getX() * physicsmpscale, mariobody:getY() * physicsmpscale,
                    mariobody:getAngle(), mpscale, mpscale, 12, 13.5)
            else
                love.graphics.draw(mariojump, mariobody:getX() * physicsmpscale, mariobody:getY() * physicsmpscale,
                    mariobody:getAngle(), mpscale, mpscale, 12, 13.5)
            end
        ...
        elseif winner == 2 then
            --luigi
            if jumpframe == false then
                love.graphics.draw(luigiidle, luigibody:getX() * physicsmpscale, luigibody:getY() * physicsmpscale,
                    luigibody:getAngle(), mpscale, mpscale, 14, 15.5)
            else
                love.graphics.draw(luigijump, luigibody:getX() * physicsmpscale, luigibody:getY() * physicsmpscale,
                    luigibody:getAngle(), mpscale, mpscale, 14, 15.5)
            end
    ...
    elseif gamestate == "failedBmulti" then
        ...
        if clearcheck then --RESULTS SCREEN INI!--
            ...
            if winner == 1 then
                ...
                mariofixture = love.physics.newFixture(mariobody, marioshape)
            ...
            elseif winner == 2 then
                ...
                luigifixture = love.physics.newFixture(luigibody, luigishape)
...
function beginContactBmulti(a, b, coll)
    local dataA = a:getUserData()
    local dataB = b:getUserData()
    collideBmulti(dataA, dataB, coll)
end
